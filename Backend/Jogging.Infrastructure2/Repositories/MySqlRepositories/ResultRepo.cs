...
public class ResultRepo : IResultRepo
{
    ...
    {
        var results = await _client.From<ResultEF>()
    ...
    {
        await _client.From<ResultEF>()
            .Upsert(_mapper.Map<List<ResultEF>>(registrations));
    }

    public async Task UpdateRunTimeAsync(int registrationId, ResultDom updatedResult)
    {
        var registration = await _client.From<ResultEF>()
